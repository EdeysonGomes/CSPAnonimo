...
The company **EstaciONE**, which owns a large parking lot in Narnia City, faces challenges in efficiently managing its parking spots, which are divided among **daily users, monthly subscribers, and rotating customers**. Each spot is exclusively reserved for a specific category, so a customer can only park if there is an available spot in their corresponding category.
...
You must upload the following to  [LMS] by **11:59 PM on 06/06/YEAR**, in the designated space:
...
4. Implement the model in [SIMULATOR] to simulate the system's behavior.
...
11. Use [SIMULATOR] to validate and simulate the system’s behavior, ensuring it meets the company’s specified requirements.
